...
Node *deleteNode(Node *head, int index){
	...
	if(index == 0){                  // SPECIAL CASE FOR DELETION AT INDEX 0
		head = head->next;
		return head;
	}	
	
	while(count < index-1 && temp != NULL){    // CHECKPOST FOR WRONG INDEX ENTERED
		temp = temp->next;
		count++;
	}
	
	if(temp != NULL){              // MAIN PART FOR INDEX OTHER THAN 0
	   Node *a = temp -> next;
	   Node *b = a -> next;
